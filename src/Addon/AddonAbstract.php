...
abstract class AddonAbstract implements AddonInterface
{
    ...
    {
        $settings = get_option("pluginforge_addon_{$this->getId()}_settings", []);
    ...
    public function updateSetting(string $key, mixed $value): void
    {
        $settings = get_option("pluginforge_addon_{$this->getId()}_settings", []);
        $settings[$key] = $value;
        update_option("pluginforge_addon_{$this->getId()}_settings", $settings);
    }
    ...
    {
        return get_option("pluginforge_addon_{$this->getId()}_settings", []);
    }
